...
class CreateWVWebiste implements DataPatchInterface
{
    ...
    public function apply(): void
    {
        ...
        $this->appState->emulateAreaCode(Area::AREA_ADMINHTML, function () {
            $website = $this->websiteFactory->create()->addData([
                'code' => WVStoreInterface::WV_WEBSITE_CODE,
                'name' => WVStoreInterface::WV_WEBSITE_NAME,
                'sort_order' => 5
            ]);
            $website->save();
            ...
            $storeGroup = $this->groupFactory->create()->addData([
                ...
                'code' => WVStoreInterface::WV_GROUP_CODE,
                'name' => WVStoreInterface::WV_GROUP_NAME,
                'root_category_id' => $this->getCategory(),
            ]);
            $storeGroup->save();

            $store = $this->storeFactory->create()->addData([
                'website_id' => $website->getId(),
                'group_id' => $storeGroup->getId(),
                'code' => WVStoreInterface::WV_STORE_CODE,
                'name' => WVStoreInterface::WV_STORE_NAME,
                'is_active' => 1,
            ]);
            $store->save();
        });
