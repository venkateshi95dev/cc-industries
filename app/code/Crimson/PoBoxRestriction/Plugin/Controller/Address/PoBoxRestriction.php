 ...
 * @date        1/23/2019
 */
namespace Crimson\PoBoxRestriction\Plugin\Controller\Address;
 ...
 * @package Crimson\PoBoxRestriction\Plugin\Controller\Address
 */
class PoBoxRestriction
{
    /**
    ...
    public function aroundExecute(FormPost $subject, \Closure $proceed)
    {
        if ($this->_helper->getModuleConfig()) {
            ...
            if ($isPoBox && isset($data['default_shipping']) && $data['default_shipping'] == 1) {
                $this->_messageManager->addErrorMessage(
                    'We don\'t ship to PO Boxes.'
                );
                $defaultUrl = $this->_urlModel->getUrl('customer/account/index', ['_secure' => true]);
