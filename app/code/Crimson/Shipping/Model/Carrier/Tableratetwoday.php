 ...
 */
class Tableratetwoday extends AbstractCarrier implements CarrierInterface
{
    /**
     * @var string
     */
    protected $_code = 'tablerate_twoday';
     ...
     */
    protected $_tableratetwodayFactory;
    ...
    public function __construct(
        ...
        TableratetwodayFactory $tableratetwodayFactory,
        ...
        $this->_tableratetwodayFactory = $tableratetwodayFactory;
    ...
    {
        return $this->_tableratetwodayFactory->create()->getRate($request);
    }
    ...
    {
        return ['bestway_twoday' => $this->getConfigData('name')];
    }
    ...
    private function createShippingMethod($shippingPrice, $cost)
    {
        ...
        $method->setMethod('bestway_twoday');
    ...
    public function processAdditionalValidation(DataObject $request)
    {
        ...
        //exclude items > 20 lbs
        if (round($request->getPackageWeight(),2) > 20.00) {
            return false;
        }
        ...
        foreach ($request->getAllItems() as $item) {
            ...
            if (!$item->getProduct()->isAvailable()) {
                ...
                $availableQty = $this->_stockItemRepository->get($item->getProduct()->getExtensionAttributes()->getStockItem()->getItemId())->getQty();
                if ($availableQty <= 0) {
                    return false;
                }
            }
