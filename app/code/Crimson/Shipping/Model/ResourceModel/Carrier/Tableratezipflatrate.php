 ...
 */
class Tableratezipflatrate extends AbstractDb
{
    ...
    /**
     * @var \Crimson\Shipping\Model\Carrier\Tableratezipflatrate
     */
    protected $carrierTableratezipflatrate;
    ...
    public function __construct(
        ...
        \Crimson\Shipping\Model\Carrier\Tableratezipflatrate $carrierTableratezipflatrate,
        ...
        $this->carrierTableratezipflatrate = $carrierTableratezipflatrate;
    ...
    {
        $this->_init('shipping_tablerate_zipflatrate', 'pk');
    }
    ...
    {
        $validDigits = $this->_helper->isZipFlatRateEnabled($request->getWebsiteId());
     ...
     * @throws LocalizedException
     */
    private function deleteByCondition(array $condition): Tableratezipflatrate
    {
        $connection = $this->getConnection();
    ...
    private function importData(array $fields, array $values)
    {
        ...
        try {
            ...
            throw new LocalizedException(
                __('Something went wrong while importing table rates Zip Flat Rate.')
            );
        }
        $connection->commit();
    }
    ...
    public function uploadAndImport(DataObject $object)
    {
         ...
         */
        if (empty($_FILES['groups']['tmp_name']['tablerate_zipflatrate']['fields']['import']['value'])) {
            return $this;
        }
        $filePath = $_FILES['groups']['tmp_name']['tablerate_zipflatrate']['fields']['import']['value'];
    ...
    {
        if ($object->getData('groups/tablerate_zipflatrate/fields/condition_name/inherit') == '1') {
            $conditionName = (string)$this->coreConfig->getValue('carriers/tablerate_zipflatrate/condition_name', 'default');
        } else {
            $conditionName = $object->getData('groups/tablerate_zipflatrate/fields/condition_name/value');
        }
    ...
    protected function _getConditionFullName($conditionName)
    {
        if (!isset($this->_conditionFullNames[$conditionName])) {
            $name = $this->carrierTableratezipflatrate->getCode('condition_name_short', $conditionName);
     ...
     * @throws LocalizedException
     */
    protected function _saveImportData(array $data): Tableratezipflatrate
    {
        if (!empty($data)) {
            $columns = [
                'website_id',
