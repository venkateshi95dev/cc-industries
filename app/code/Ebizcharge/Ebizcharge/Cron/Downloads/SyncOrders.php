...
class SyncOrders
{
    ...
    /**
     * Execute method fetch Orders data from EBizCharge Hub
     *
     * @return void
     */
    public function execute()
    {
        ...
        try {
            $this->_ebizchargeLogger->addInfo(__(
                'Orders download process started to fetch the records from EBizCharge Hub'
            ));
            /**
             * Downloading Shell process to download orders
             */
            $this->_shellCommand->shellProcessDownloadOrders();
            $this->_ebizchargeLogger->addInfo(__(
                'Orders download process Completed to fetch records from EBizCharge Hub'
            ));
        } catch (LocalizedException $exception) {
            $this->_ebizchargeLogger->addCritical(__(
                "Exception occurred during fetching orders from EBizCharge Hub. " . $exception->getMessage()
            ));
        }
    }
}
