    ...
    public function execute()
    {
        try {
            $this->_ebizchargeLogger->addInfo(__(
                'Orders upload process has started to upload the records to EBizCharge Gateway'
            ));

            /**
             * run shell command for Uploading Orders
             */
            $this->_shellCommand->shellProcessUploadOrders();

            $this->_ebizchargeLogger->addInfo(__(
                'Orders upload process has completed to upload the records to EBizCharge Gateway'
            ));
        } catch (LocalizedException $exception) {
            $this->_ebizchargeLogger->addCritical(__(
                "Exception occurred during uploading Orders to EBizCharge Hub " . $exception->getMessage()
            ));
        }
    }
}
