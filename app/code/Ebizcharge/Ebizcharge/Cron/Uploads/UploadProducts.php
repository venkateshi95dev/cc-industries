...
class UploadProducts
{
    ...
    /**
     * Execute method to upload Products EBizCharge Hub
     *
     * @return void
     */
    public function execute()
    {
        ...
        try {
            $this->_ebizchargeLogger->addInfo(__(
                'Products|Items upload process has started to upload the records to EBizCharge Gateway'
            ));

            /**
             * run shell command for Uploading products
             */
            $this->_shellCommand->shellProcessUploadProducts();

            $this->_ebizchargeLogger->addInfo(__(
                'Products|Items upload process has completed to upload the records to EBizCharge Gateway'
            ));
        } catch (LocalizedException $exception) {
            $this->_ebizchargeLogger->addCritical(__(
                "Exception occurred during uploading Products|Items to EBizCharge Hub " . $exception->getMessage()
            ));
        }
    }
}
