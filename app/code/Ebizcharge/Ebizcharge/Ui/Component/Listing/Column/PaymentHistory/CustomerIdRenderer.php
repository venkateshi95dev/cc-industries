 ...
 */
class CustomerIdRenderer extends Column
{
    /**
     * @var Config
     */
    ...
    public function prepareDataSource(array $dataSource): array
    {
        if (isset($dataSource['data']['items'])) {
            $customerId = $this->getData('name');
            foreach ($dataSource['data']['items'] as & $item) {
                if (isset($item[$customerId])) {
                    $item[$customerId] = $this->getCustomerIdHtml($item[$customerId]);
                }
            }
        }
     ...
     * @return string
     */
    private function getCustomerIdHtml($customerId): string
    {
        $cssClass = 'customer-male-icon';
        return '<span ><span> ' . $customerId . '</span></span>';
    }
}
