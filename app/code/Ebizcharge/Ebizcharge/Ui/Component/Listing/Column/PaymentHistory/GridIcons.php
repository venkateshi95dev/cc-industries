 ...
 * Class GridIcons
 */
class GridIcons extends Column
{
    /**
     * @var Config
     */
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item['customer_email'])) {
                    ...
                    $item['customer_email'] = $this->getGridIconsHtml($item['customer_email'], $columnName);
                }
                if (isset($item['email'])) {
                    $columnName = 'email';
                    $item['customer_email'] = $this->getGridIconsHtml($item['email'], $columnName);
                }
            }
        }
    ...
    /**
     * Get status html as per getGridIconsHtml
     *
     * @param string $gridLabel
     * @param string $columnName
     * @return string
     */
    private function getGridIconsHtml($gridLabel = '', $columnName = ""): string
    {
        $cssClass = 'customer-email-icon';
        if ($columnName == 'customer_email') {
            $cssClass = 'customer-email-icon';
        }
