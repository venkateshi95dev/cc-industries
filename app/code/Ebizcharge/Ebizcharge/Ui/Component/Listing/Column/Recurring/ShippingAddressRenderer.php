...
/**
 * Recurring Customer Shipping Address Ui Column Component
 *
 * Class CustomerEmail
 */
class ShippingAddressRenderer extends Column
{
    /**
     * @var Escaper
     */
    ...
    public function prepareDataSource(array $dataSource)
    {
        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                $customerShippingAddressId = $item['billing_address_id'];
                $columnName = nl2br($this->_escaper->escapeHtml($item[$this->getData('name')]));
                $item[$this->getData('name')] = $this->getLabelInfo($columnName, $customerShippingAddressId);
            }
        }
     ...
     * @param null|mixed $customerShippingAddressId
     * @return string
     */
    private function getLabelInfo($gridLabel, $customerShippingAddressId = null): string
    {

        $addressHtml = $customerShippingAddressId;
        $cssClass = 'bank-account-card-icon';

        try {
            /** @var  $customer */
            $customerAddress = $this->_customerFactory->create()->loadCustomerAddressById($customerShippingAddressId);
            ...
            return '<span class="' . $cssClass . '"><span>' . $customerShippingAddressId . '</span></span>';
        }
    }
}
