...
abstract class UpsAPI extends AbstractModel
{
     ...
     * @return void
     */
    public function setNewAccesKeys($clientApiKey, $clientSecret)
    {
    ...
    public function getApiKey()
    {
        ...
        curl_setopt_array($curl, [
            CURLOPT_HTTPHEADER => [
                ...
                "Authorization: Basic " . base64_encode($this->clientApiKey . ":" . $this->clientSecret)
            ],
     ...
     * @return array|mixed
     */
    public function sendRequestKey($request)
    {

        $dataSec = $this->getApiKey();
        $arrayData = json_decode($dataSec, true);
        ...
        curl_setopt_array($curl, [
            CURLOPT_HTTPHEADER => [
                "Authorization: Bearer " . $arrayData['access_token'],
                "Content-Type: application/json"
            ],
            CURLOPT_POSTFIELDS => json_encode($request),
            CURLOPT_URL => "https://onlinetools.ups.com/api/addressvalidation/" . $version . "/" . $requestoption . "?" . http_build_query($query),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => "POST",
        ]);
        ...
        $this->response_array['AddressValidationResponse'] = json_decode($response, true)['XAVResponse']['Candidate'];
        $this->response_array['AddressValidationResponse']['Response'] = json_decode($response, true)['XAVResponse']['Response'];
