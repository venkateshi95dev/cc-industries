...
class Validation extends AbstractValidation
{
    ...
    public function validateAddress()
    {
        ...
        if($this->helper->getNewAccessKey()){
            $this->validator->setNewAccesKeys($this->helper->getClientApiKey(), $this->helper->getClientSecretKey());
            $json = $this->validator->buildJsonRequest();
            $response = $this->validator->sendRequestKey($json);
        }else{
            $this->setupConfigUPS();
    ...
    private function getUpsCandidates($response)
    {
        ...
        if (isset($addresses_array['AddressClassification']) || isset($response['AddressClassification'])) {
    ...
    public function getEnable()
    {
        $login = $this->helper->getUpsLogin();
        $pass = $this->helper->getUpsPassword();
        $key = $this->helper->getUpsAccessKey() ?? $this->helper->getClientApiKey();
        $secretKey = $this->helper->getClientSecretKey();

        return !(empty($key) || empty($login) || empty($pass) || empty($secretKey));
    }
}
