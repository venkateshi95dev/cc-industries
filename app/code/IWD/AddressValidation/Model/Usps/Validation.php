...
class Validation extends AbstractValidation
{
    ...
    public function __construct(
        ...
        USPSAddressRest $USPSAddressRest,
        USPSAddressVerify $USPSAddressVerify,
        USPSAddressVerifyRest $USPSAddressVerifyRest,
        ...
        $this->USPSAddress = $this->helper->getUspsApiType() ? $USPSAddressRest :  $USPSAddress;
        $this->USPSAddressVerify = $this->helper->getUspsApiType() ? $USPSAddressVerifyRest :  $USPSAddressVerify;
    }
    ...
    private function convertAddressToUSPSAddress()
    {
        ...
        if(!$this->helper->getUspsApiType()){
            $this->USPSAddress->setZip4($zip4);
            $this->USPSAddress->setApt($apt);
        }

        $this->USPSAddress->setAddress($street)
            ->setCity($city)
            ->setState($state)
            ->setZip5($zip5);
    ...
    private function getUspsVerifier()
    {
        ...
        $apiKey = $this->helper->getUspsAPi();
        $secretKey = $this->helper->getUspsSecret();


        if( $this->helper->getUspsApiType()){
            $this->USPSAddressVerify->setApiKey($apiKey);
            $this->USPSAddressVerify->setSecretKey($secretKey);
        }else{
    ...
    private function validateResponse($verify)
    {
        ...
        $key = $this->helper->getUspsApiType() ? 'address' : 'AddressValidateResponse';
    ...
    private function parseResponse($verify)
    {
        ...
        $candidate = $this->helper->getUspsApiType() ? $response['address'] : $response['AddressValidateResponse']['Address'];
        $validCandidate = $this->helper->getUspsApiType() ? $this->parseNewUspsCandidate($candidate) : $this->parseUspsCandidate($candidate);
    ...
    private function parseNewUspsCandidate($candidate)
    {
        if (!isset($candidate['streetAddress']) || empty($candidate['streetAddress']) ||
            !isset($candidate['city']) || empty($candidate['city']) ||
            !isset($candidate['state']) || empty($candidate['state']) ||
            !isset($candidate['ZIPCode']) || empty($candidate['ZIPCode'])
        ) {
            return false;
        }
        ...
        $postcode = $candidate['ZIPCode'];
        if (isset($candidate['ZIPPlus4']) && !empty($candidate['ZIPPlus4'])) {
            $postcode .= '-' . $candidate['ZIPPlus4'];
        }

        $street = $candidate['streetAddress'];
        if (isset($candidate['streetAddressAbbreviation']) && !empty($candidate['streetAddressAbbreviation'])) {
            $street .= ' ' . $candidate['streetAddressAbbreviation'];
        }
