...
namespace Silk\Coker\Model\Indexer\Fulltext\Action;
...
class DataProvider extends \Magento\CatalogSearch\Model\Indexer\Fulltext\Action\DataProvider
{
 /**
    ...
    public function prepareProductIndex($indexData, $productData, $storeId)
    {
        ...
        foreach ($this->getSearchableAttributes('static') as $attribute) {
            ...
            if (isset($productData[$attributeCode])) {
                ...
                $value = $this->doAttributeValue($attribute->getId(), $productData[$attributeCode], $storeId);
                if ($value) {
        ...
        foreach ($indexData as $entityId => $attributeData) {
            foreach ($attributeData as $attributeId => $attributeValue) {
                $value = $this->doAttributeValue($attributeId, $attributeValue, $storeId);
                if (!empty($value)) {
     ...
     * @return string
     */
    private function doAttributeValue($attributeId, $valueId, $storeId)
    {
        ...
        if($attribute->getAttributeCode()=="tire_size"){
            if(strpos($value,'/') !== false){
                $value12 = explode("/",$value);
                $value13 ="";
                foreach ($value12 as $key => $val) {
                   $value13.= $val." ";
                }
                $value = $value." ".$value13;
            ...
            if(strpos($value,'R') !== false){
                $value2 = str_replace("R", "", $value);
                $value = $value." ".$value2;
            }
            if(preg_match("/[0-9]{3}[\/][0-9]{3}[-][0-9]{2}[\/][0-9]{2}/",$value)){
                $value4 = str_replace("-", " ", $value);
                $value5 = str_replace("/", " ", $value4);
                $value6 = explode(" ",$value5);
                $value7 = $value6[0]."-".$value6[3]." ".$value6[0]."-".$value6[2]." ".$value6[1]."-".$value6[3]." ".$value6[1]."-".$value6[2];
                $value = $value." ".$value7;
            }
            if(preg_match("/[0-9]{3}[\/][0-9]{3}[-][0-9]{2}/",$value)){
                $value8 = str_replace("-", " ", $value);
                $value9 = str_replace("/", " ", $value8);
                $value10 = explode(" ",$value9);
                $value11 = $value10[0]."-".$value10[2]." ".$value10[1]."-".$value10[2];
                $value = $value." ".$value11;
            }
            if(strpos($value,'-') !== false){
            ...
            if(strpos($value,'x') !== false){
                $value14 = str_replace("x", "", $value);
                $value = $value." ".$value14;
            }
            if(strpos($value,'X') !== false){
                $value15 = str_replace("X", "", $value);
                $value = $value." ".$value15;
            }
     ...
     /*   $value = str_replace("X", "replacecha", $value);
        $value = str_replace("x", "replacecha", $value);*/
        $value = str_replace("/", " ", $value);
        $value = str_replace('"', "", $value);
        // $value = str_replace(".", "renamedian", $value);
        $value = str_replace(".", ".", $value);
        $value = str_replace(",", "", $value);
        $value = str_replace("|", "", $value);
        return $value;
